...
openai.api_key = "A"
# Define the symptom list and synonyms in one file.
symptom_list = [
...
def pick_symptom_variant():
    """Randomly choose a canonical symptom and possibly substitute with one of its synonyms."""
    symptom = random.choice(symptom_list)
    if symptom in symptom_synonyms and random.choice([True, False]):
        return symptom, random.choice(symptom_synonyms[symptom])
    return symptom, symptom

def build_prompt(details):
    """
    Build a GPT-3.5 prompt that instructs the model to generate a natural sentence that
    includes the given symptom detail. The output must be a valid JSON object with two keys:
      "sentence": the generated natural sentence,
      "symptom": the canonical symptom (from symptom_list).
    Do not use any fixed phrase; create varied natural language.
    Include an instruction to avoid overlapping or ambiguous entity spans.
    """
    prompt = (
        "Generate a natural sentence that mentions a symptom in context. "
        "Include details such as age, duration, etc. "
        "Then output a JSON object with two keys:\n"
        "  - \"sentence\": the full natural sentence\n"
        "  - \"symptom\": the canonical symptom label (one of: " + ", ".join(symptom_list) + ")\n"
        "Make sure the sentence is varied and natural, and that the symptom mentioned corresponds clearly to the canonical symptom. "
        "Do not include any extra text, only the valid JSON object.\n\n"
        "Details:\n"
        f"  - Symptom mention (variant): {details['variant']}\n"
        f"  - Canonical symptom: {details['canonical']}\n"
    )
    return prompt

def generate_example():
    """Generate one training example via GPT-3.5."""
    canonical, variant = pick_symptom_variant()
    details = {"canonical": canonical, "variant": variant}
    prompt = build_prompt(details)
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are an expert at generating natural language training data for sentence embeddings."},
        ...
        print("Error generating example:", e)
...
if __name__ == "__main__":
    ...
    for i in range(num_examples):
        ...
        if ex:
            ...
            print(f"Failed example {i+1}/{num_examples}")
        # Optionally, sleep a short duration to avoid rate limits.
        time.sleep(1)

    # Save the examples to sbert_symptom_data.json
    with open("sbert_symptom_data.json", "w", encoding="utf8") as f:
        json.dump(examples, f, indent=2)

    print(f"Finished: Generated {len(examples)} training examples in sbert_symptom_data.json")
