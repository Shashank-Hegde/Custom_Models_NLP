...
model = SentenceTransformer("custom_sbert_model")

nlp = spacy.load("en_core_web_sm")
# Define our canonical symptom list and synonyms.
symptom_list = [
...
candidate_embeddings = model.encode(symptom_candidates, convert_to_tensor=True)

# Define a set of negation phrases to look for.
NEGATION_PATTERNS = [
    r'\bdo not have\b',
    r'\bdoes not have\b',
...
def contains_negation(sentence, candidate):
    """
    A simple heuristic: Return True if one of our negation patterns
    appears in the sentence in proximity to the candidate string.
    We'll look for the candidate preceded (or followed) by a negation phrase.
    """
    sentence_lower = sentence.lower()
    candidate_lower = candidate.lower()
    for pattern in NEGATION_PATTERNS:
        # Check if pattern is found followed by candidate, or candidate preceded by pattern.
...
def extract_symptoms(text, threshold=0.6):
    """
    Split the input text into sentences, encode each using the SBERT model,
    compare to the reference candidate embeddings, and return a list of matched symptom
    candidates for each sentence that are not in a negated context.
    """
    sentences = sent_tokenize(text)
    extracted = []  # List of dictionaries, each for one sentence.

    # Compute embeddings for each sentence.
    sentence_embeddings = model.encode(sentences, convert_to_tensor=True)

    for sentence, sent_emb in zip(sentences, sentence_embeddings):
        # Get cosine similarities with all candidate embeddings.
        cosine_scores = util.cos_sim(sent_emb, candidate_embeddings)[0]
        # Get all candidates that exceed the threshold.
        indices = [i for i, score in enumerate(cosine_scores) if score.item() >= threshold]
        matched_candidates = []
        for i in indices:
            candidate = symptom_candidates[i]
            # Check negation in the sentence.
            if contains_negation(sentence, candidate):
                # Skip this candidate.
                continue
            matched_candidates.append((candidate, cosine_scores[i].item()))
        if matched_candidates:
            # You could choose the candidate with the highest score or keep them all.
            extracted.append({
                "sentence": sentence,
                "matched_symptoms": matched_candidates
            })
    return extracted

if __name__ == "__main__":
    sample_text = (
        "Also, I have noticed high temperature and dizziness recently. "
        "I have cough since yesterday. "
        "I have a back spasm and severe headache, but I do not have a migraine. "
        "I experience back spasm frequently during exercise."
    )

    results = extract_symptoms(sample_text, threshold=0.6)
    print("Extracted Symptoms:")
    for res in results:
        print(f"Sentence: {res['sentence']}")
        for candidate, score in res["matched_symptoms"]:
            print(f"  Matched symptom: {candidate} (similarity: {score:.2f})")
        print("-----")
