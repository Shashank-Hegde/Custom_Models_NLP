...
# Locations (sample of Indian cities/towns)
location_list = [
...
# Age descriptors
age_terms = ['20-year-old', '30-year-old', '40-year-old', '50-year-old']

# Duration expressions for the symptom
duration_list = [
    'for 3 days', 'since yesterday', 'for one week', 'for a month'
]

# --- End of variable definitions ---

def pick_variant(symptom):
    """Randomly pick either the canonical symptom or one of its synonyms (if available)."""
    if symptom in symptom_synonyms and random.choice([True, False]):
        return random.choice(symptom_synonyms[symptom])
    return symptom

def build_gpt_prompt(details):
    """
    Build a GPT prompt that instructs GPT-3.5 to generate a natural sentence that
    includes the given details. The output must be a JSON array in spaCy training format:
      [ "Sentence...", {"entities": [[start, end, "LABEL"], ...] } ]
    Ensure that:
      - The character offsets for each entity match exactly the location of that entity in the sentence.
      - **No two entities overlap.**
    Do not use fixed phrasing; let GPT generate varied, natural language.
    """
    prompt = (
        "Generate a natural language training example that includes the following details (in a natural and varied way):\n"
        f" • Symptom: {details['SYMPTOM']}\n"
        f" • Medication: {details['MEDICATION']}\n"
        f" • Location: {details['LOCATION']}\n"
        f" • Age: {details['AGE']}\n"
        f" • Gender: {details['GENDER']}\n"
        f" • Duration: {details['DURATION']}\n\n"
        "Output the sentence and its entity annotations in a JSON array that follows this spaCy training format:\n"
        "[ \"<Sentence>\", {\"entities\": [[start, end, \"SYMPTOM\"], "
        "[start, end, \"MEDICATION\"], [start, end, \"LOCATION\"], "
        "[start, end, \"AGE\"], [start, end, \"GENDER\"], [start, end, \"DURATION\"]]} ]\n\n"
        "Ensure that the character offsets for each entity match exactly the text in the sentence and that "
        "no two entities overlap (each character may belong to only one entity). Do not include any extra text."
    )
    return prompt

def generate_training_example():
    """Generates one training example using GPT-3.5."""
    # Randomly select details
    symptom_canonical = random.choice(symptom_list)
    symptom_variant = pick_variant(symptom_canonical)
    medication = random.choice(medications_list)
    location = random.choice(location_list)
    gender = random.choice(gender_list)
    age = random.choice(age_terms)
    duration = random.choice(duration_list)

    details = {
        "SYMPTOM": symptom_variant,
    ...
    prompt = build_gpt_prompt(details)

    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are an expert that generates natural language training data for spaCy."},
        ...
        training_example = json.loads(generated_text)
        return training_example
    except Exception as e:
        print("Error generating training example:", e)
...
if __name__ == "__main__":
    ...
    for i in range(num_examples_to_generate):
        ...
        if example:
            ...
            print(f"[GPT] Generated example {i+1}/{num_examples_to_generate}")

    # Save the GPT-generated training examples to a JSON file.
    with open("gpt_training_data.json", "w", encoding="utf8") as f:
        json.dump(generated_examples, f, indent=2)

    print(f"Finished: Generated {len(generated_examples)} training examples.")
